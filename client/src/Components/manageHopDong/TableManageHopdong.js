...
//     return axios.get(`http://127.0.0.1:8080/api/bill?status=${status}`)
//       .then(response => {
...
//     let cloneListHoadon = _.cloneDeep(listHoadon);
//     cloneListHoadon = cloneListHoadon.filter(item => item.billId !== hoadon.billId);
//     setListHoadon(cloneListHoadon);
//   }
...
//     result.push(['Phòng số',['Ngày lập'],['Hạn thanh toán'],['Tổng hóa đơn'],['Phương thức'],['Tình trạng']])
//       listHoadon.map((item) =>{
//         let arr = [];
//         arr[0] = item.roomId;
...
// return (
//   <>
//    <div className="my-3 add-new" style={{ display: 'flex', alignItems: 'center' }}>
//   <span><b>Danh sách Hoá đơn: </b></span>
//   <select
//     value={status}
//     onChange={(event) => {
...
//     }}
//   >
//     <option value="all" style={{ backgroundColor: '#ccc', color: 'black', textAlign: 'center' }}>Chọn tình trạng ...</option>
// <option value="đã thanh toán" style={{ backgroundColor: '#ccc', color: 'black' }}>Đã thanh toán</option>
// <option value="chưa thanh toán" style={{ backgroundColor: '#ccc', color: 'black' }}>Chưa thanh toán</option>
//   </select>
...
//         </CSVLink>
//         <button className='btn btn-success' onClick={() => setIsShowModalAddDiennuoc(true)}>
//           <i class="fa-solid fa-plug-circle-plus"></i> Thêm Điện Nước
//         </button>
//         <button className='btn btn-success' onClick={() => setIsShowModalAddHoadon(true)}>
...
//       <input className='form-control' placeholder='Tìm kiếm hóa đơn ... ' onChange={(event) => handleSearchHoadon(event.target.value)} />
//     </div>
...
//       </thead>
//       <tbody>
//         {listHoadon && listHoadon.map((item, index) => (
//           <tr key={`hoadon-${index}`}>
//             <td>{item.roomId}</td>
...
//             <td>{item.status}</td>
//             <td>
//               <button className='btn btn-warning mx-3' onClick={() => handleEditHoadon(item)}>Edit</button>
//               <button className='btn btn-danger' onClick={() => handDeleteHoadon(item)}>Delete</button>
//               <button className='btn btn-success mx-3' onClick={() => handDetailHoadon(item)}>Chi tiết</button>
//             </td>
//           </tr>
//         ))}
//       </tbody>
